...
public class GetterTest {
	 ...
	 */
	public static boolean verifyTestUser(String applicationRoot, String userName, String password, String theClass)
			throws SQLException {
		boolean result = false;
		...
		if (user != null && !user[0].isEmpty()) {
			...
			TestProperties.failAndPrint("Could not verify that " + userName + " could authenticate at all");
		}
			...
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, dataStorageLessonId, userId).isEmpty()) {
						...
						String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot, dataStorageLessonId,
								userId, "Feedback is Disabled", 1, 1, 1);
						if (markLevelCompleteTest != null) {
							String checkPlayerResultTest = Getter.checkPlayerResult(applicationRoot,
									dataStorageLessonId, userId);
	...
	public void testCheckPlayerResultWhenModuleNotComplete() {
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					// Simulate user Opening Level
					if (!Getter.getModuleAddress(applicationRoot, contentProviderLeakage,
							Getter.getUserIdFromName(applicationRoot, userName)).isEmpty()) {
						String checkPlayerResultTest = Getter.checkPlayerResult(applicationRoot, contentProviderLeakage,
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (test != null) {
					...
					fail("Function says User has opened module they should not have opened by default"); // User Should
																											// not have
																											// completed
																											// this
																											// module by
	...
	public void testCheckPlayerResultWhenModuleWhenOpened() {
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					// Simulate user Opening Level
					if (!Getter.getModuleAddress(applicationRoot, csrfChallengeThree,
							Getter.getUserIdFromName(applicationRoot, userName)).isEmpty()) {
						String test = Getter.checkPlayerResult(applicationRoot, csrfChallengeThree,
								Getter.getUserIdFromName(applicationRoot, userName));
						if (test == null) {
							fail("Function says " + userName + " has not opened module"); // User Should have opened and
			...
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!modules.isEmpty()) // Some Modules were included in response
					{
						...
						int numberofChallengesReturned = (modules.length()
								- modules.replace("class='lesson'", "").length()) / "class='lesson'".length();
						if (numberofChallengesReturned > totalNumberOfModulesInShepherd) {
							log.debug("PASS: Found " + numberofChallengesReturned + " modules");
							return;
						} else {
							log.debug("Too Few Challenges Returned to pass: " + numberofChallengesReturned
									+ " returned. Expected at least:" + totalNumberOfModulesInShepherd);
				...
				if (Setter.closeAllModules(applicationRoot)) {
					...
					if (!modules.isEmpty()) // Some Modules were included in response
					{
						...
						if (!(numberofChallengesReturned > 0)) {
							...
							log.debug("Too Many Challenges Returned to pass: " + numberofChallengesReturned
									+ " returned");
	...
	public void testGetCsrfForumWithIframe() {
		...
		try {
			...
			if (verifyTestUser(applicationRoot, userName, userName, classId)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					// Simulate user Opening Level
					if (!Getter.getModuleAddress(applicationRoot, moduleId,
							Getter.getUserIdFromName(applicationRoot, userName)).isEmpty()) {
			...
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, dataStorageLessonId, userId).isEmpty()) {
						...
						String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot, dataStorageLessonId,
								userId, feedbackSearchCode, 1, 1, 1);
						if (markLevelCompleteTest != null) {
							String checkPlayerResultTest = Getter.checkPlayerResult(applicationRoot,
									dataStorageLessonId, userId);
							log.debug("checkPlayerResultTest" + checkPlayerResultTest);
							if (checkPlayerResultTest == null) {
								log.debug(
										"Checking to see if the feedback is included in the getFeeback response for the module");
								...
								if (feedback.indexOf(feedbackSearchCode) > -1) {
									...
									log.fatal("User's Feedback '" + feedbackSearchCode + "' was not found in: "
											+ feedback);
	...
	public void testGetIncrementalModulesWithModulesClosed() {
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.closeAllModules(applicationRoot)) {
					String incrementalModules = Getter.getIncrementalModules(applicationRoot, userId, lang,
							"testingCSRFtoken");
					if (incrementalModules.indexOf("You've Finished!") > -1) // IF no modules are open, this is the
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					String incrementalModules = Getter.getIncrementalModules(applicationRoot, userId, lang,
							"testingCSRFtoken");
					if (incrementalModules.indexOf("Completed") == -1) // User should not have completed any modules.
																		// The Completed Button should not be present
					{
						if (incrementalModules.indexOf(lowestRankModuleId) > -1) // The only module Id to be returned
																					// should be this one as it is the
																					// first presented (Lowest
																					// Incremental Rank)
						{
							if (incrementalModules.indexOf("Get Next Challenge") > -1) // This is the English string
																						// that should be included with
																						// the lang submitted in this
																						// unit test
							{
								log.debug(
										"PASS: Incremental Menu Appears to have Rendered correctly with the Preconditions of this test");
								return;
							} else {
	...
	public void testGetIncrementalModulesWithOneModuleComplete() {
		...
		String lowestRankModuleId = "0dbea4cb5811fff0527184f99bd5034ca9286f11"; // This should be changed if an easier
																				// module is made
		String secondLowestRankModuleId = "b9d82aa7b46ddaddb6acfe470452a8362136a31e"; // This should be changed if an
			...
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, lowestRankModuleId, userId).isEmpty()) {
						// Then, Mark the Challenge Complete for user (Insecure Data Storage Lesson)
						String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot, lowestRankModuleId,
								userId, "Feedback is Not Enabled", 1, 1, 1);
						if (markLevelCompleteTest != null) {
							String checkPlayerResultTest = Getter.checkPlayerResult(applicationRoot, lowestRankModuleId,
									userId);
							log.debug("checkPlayerResultTest" + checkPlayerResultTest);
							if (checkPlayerResultTest == null) {
								String incrementalModules = Getter.getIncrementalModules(applicationRoot, userId, lang,
																					...
																					// should be present
								{
																								...
																								// this one
									{
										if (incrementalModules.indexOf(secondLowestRankModuleId) > -1) {
											...
											fail("The Module Id Returned to be Completed Next was not the Known 2nd Level. Ie not: "
													+ secondLowestRankModuleId);
										}
									} else {
										fail("The Module Id Returned was not the Known First Level. Ie not: "
												+ lowestRankModuleId);
									}
								} else {
									fail("CTF Menu Appears as if User Has Completed Modules When They Have Not");
								...
								fail("checkPlayerResultTest says user has not completed module"); // Even though this
																									// test just marked
																									// it as Completed
							}
						} else
	...
	public void testGetIncrementalModulesWithoutScriptWithModulesClosed() {
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				// Close all Modules First
				if (Setter.closeAllModules(applicationRoot)) {
					String incrementalModules = Getter.getIncrementalModulesWithoutScript(applicationRoot, userId, lang,
							"testingCSRFtoken");
					if (incrementalModules.indexOf("You've Finished!") > -1) // IF no modules are open, this is the
																				// expected leading string
					{
						if (!incrementalModules.endsWith(";</script>")) {
							log.debug(
									"PASS: Incremental Menu Appears to have Rendered correctly with the Preconditions of this test without ending in the button script");
							return;
						} else {
							log.debug("incrementalModules returned: " + incrementalModules);
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, lowestRankModuleId, userId).isEmpty()) {
						...
						if (markLevelCompleteTest != null) {
							...
							log.debug("checkPlayerResultTest" + checkPlayerResultTest);
							if (checkPlayerResultTest == null) {
								String incrementalModules = Getter.getIncrementalModulesWithoutScript(applicationRoot,
										userId, lang, "testingCSRFtoken");
								if (incrementalModules.indexOf("Completed") > -1) // User should have completed one
																					// module. The Completed Button
																					// should be present
								{
	...
	/**
	 * Tests to ensure that user can only see their data in the scoreboard, and
	...
	public void testGetJsonScoreClassSpecific() {
		...
		String insecureDirectObjectRefLesson = "0dbea4cb5811fff0527184f99bd5034ca9286f11"; // Direct Object Reference
																							// Module
		try {
			try {
				classId = findCreateClassId(className);
			...
			if (verifyTestUser(applicationRoot, userName, userName, classId)
				...
				// Open all Modules First
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot,
							insecureDirectObjectRefLesson, userId, "Feedback is Disabled", 1, 1, 1);
					if (markLevelCompleteTest != null)
						markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot,
								insecureDirectObjectRefLesson, otherUserId, "Feedback is Disabled", 1, 1, 1);
					else
	...
	/**
	 * Test to ensure users that have not scored any points, or are on negative
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (Setter.updateUserPoints(applicationRoot, otherUserId, -5)) {
						...
						if (scoreboardData.isEmpty()) {
							log.debug(
									"PASS: The Scoreboard response was empty. Therefore the users are not valid to be returned");
							return; // PASS
						}
	...
	/**
	 * Test to see if Score board returns score for entire user base regardless of
	 * class
	 */
	@Test
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (markLevelCompleteTest != null) {
						...
						for (int i = 0; i < scoreboardJson.size(); i++) {
							...
							if (scoreRowJson.get("username").toString().compareTo(otherUserName) == 0) // Therefore not
																										// encoded for
																										// HTML
							{
								fail("Found " + otherUserName + " in scoreboard");
			...
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, inscureDirectObjectLesson, userId).isEmpty()) {
						String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot,
								inscureDirectObjectLesson, userId, "Feedback is Disabled", 1, 1, 1);
						if (markLevelCompleteTest != null) {
							...
							{
								if (lessonsMenu.indexOf(inscureDirectObjectLesson) > -1) // This module should be in the
																							// response
								{
									if (lessonsMenu
											.indexOf("<img src='css/images/completed.png'/><a class='lesson' id='"
													+ inscureDirectObjectLesson) > -1) // This module should be returned
																						// as completed
									{
										if (lessonsMenu
												.indexOf("<img src='css/images/uncompleted.png'/><a class='lesson' id='"
														+ poorDataValidationLesson) > -1) {
											...
											{
												log.debug(
														"PASS: GetLessons Menu Appears to have Rendered correctly with the Preconditions of this test");
												return;
											} else {
										...
										log.fatal(
												"Could not detect completed Icon beside Insecure Direct Object Reference Lesson");
										log.error(
												"Could not find : <img src='css/images/completed.png'/><a class='lesson' id='"
														+ inscureDirectObjectLesson + " in " + lessonsMenu);
										fail("Completed Module Did not Have Completed Symbol");
									}
								} else {
									log.fatal("Could not find Insecure Direct Object References ModuleID in: "
											+ lessonsMenu);
	...
	/**
	 * Test to see if correct meny is returned from getLessons when modules are
	 * closed
	 */
	@Test
		...
		try {
			if (verifyTestUser(applicationRoot, userName, userName)) {
				...
				if (Setter.closeAllModules(applicationRoot)) {
					...
					{
						if (lessonsMenu.indexOf("No lessons found") > -1) // English string should exist in output based
	...
	public void testGetModuleCategory() {
		...
		if (Getter.getModuleCategory(applicationRoot, insecureCryptoLesson)
				.compareTo("Insecure Cryptographic Storage") != 0) {
	...
	public void testGetModuleHash() {
		...
		if (Getter.getModuleHash(applicationRoot, insecureCryptoLesson)
				.compareTo("if38ebb58ea2d245fa792709370c00ca655fded295c90ef36f3a6c5146c29ef2") != 0) {
	...
	public void testGetModuleIdFromHash() {
		...
		if (Getter.getModuleIdFromHash(applicationRoot, Getter.getModuleHash(applicationRoot, insecureCryptoLesson))
				.compareTo(insecureCryptoLesson) != 0) {
	...
	/**
	 * Function should return the entire list of modules regardless of status in
	 * options tags
	 */
	@Test
	...
	public void testGetModuleStatusMenu() {
		...
		if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
			...
			} else if (moduleStatusMenu.indexOf("id='toOpen'></select></td>") < 0) // Should be empty as all modules
																					// should be open
			{
				...
				int endOfToCloseMenu = moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toClose'>"))
						.indexOf("</select>")
						+ (moduleStatusMenu.length()
								- moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toClose'>")).length());
				String toCloseList = moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toClose'>"),
						endOfToCloseMenu);
	...
	public void testGetModuleStatusMenuWhenClosed() {
		...
		if (Setter.closeAllModules(applicationRoot)) {
			...
			if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
				...
				log.fatal("No Close Menu Detected in Output: " + moduleStatusMenu);
				fail("No Close Meny Detected in Output");
			} else if (moduleStatusMenu.indexOf("id='toClose'></select></td>") < 0) // Should be empty as all modules
																					// should be closed
			{
				...
				int endOfToOpenMenu = moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toOpen'>"))
						.indexOf("</select>")
						+ (moduleStatusMenu.length()
								- moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toOpen'>")).length());
				String toOpenList = moduleStatusMenu.substring(moduleStatusMenu.indexOf("id='toOpen'>"),
						endOfToOpenMenu);
	...
	public void testGetModuleStatusMenuWhenMobileOnlyOpen() {
		...
		if (Setter.openOnlyMobileCategories(applicationRoot)) {
			...
			if (moduleStatusMenu.indexOf("<tr><th>To Open</th><th>To Close</th></tr><tr>") == -1) {
				...
				log.fatal("No Close Menu Detected in Output: " + moduleStatusMenu);
				fail("No Close Meny Detected in Output");
			} else if (moduleStatusMenu.indexOf("id='toClose'></select></td>") > 0) // Should not be empty as Web Levels
																					// should be closed
			{
			...
			} else if (moduleStatusMenu.indexOf("id='toOpen'></select></td>") > 0) // Should not be empty as Mobile
																					// Levels should be open
			{
				...
				if (toCloseList.indexOf(dataStorageLessonId) == -1) {
					...
					int numberOfMobileLevelsOpen = (toCloseList.length() - toCloseList.replace("<option", "").length())
							/ "<option".length();
					log.debug(numberOfMobileLevelsOpen + " mobile levels detected");
					int numberOfWebLevelsClosed = (toOpenList.length() - toOpenList.replace("<option", "").length())
							/ "<option".length();
					...
					try {
						...
						prepStatement = conn.prepareStatement(
								"SELECT COUNT(*) FROM modules WHERE " + Setter.webModuleCategoryHardcodedWhereClause);
					...
					if (mobileDbModuleCount != numberOfMobileLevelsOpen) {
						...
						fail("There are " + numberOfWebLevelsClosed + " web levels in open list, but there are "
								+ webDbModuleCount + " in the DB");
						// This may mean that the where clause in the setter method is not correct and
						// needs to be updated
					} else if ((mobileDbModuleCount + webDbModuleCount) != (numberOfMobileLevelsOpen
		...
		try {
			...
			if (verifyTestUser(applicationRoot, userName, userName, classId)
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (Getter.getModuleAddress(applicationRoot, insecureDirectObjectRefLesson, userId).isEmpty()) {
					...
					} else if (Getter.getModuleAddress(applicationRoot, insecureDirectObjectRefLesson, otherUserId)
							.isEmpty()) {
						fail("Could not Simulate Opening Level for User 1");
					} else {
						String markLevelCompleteTest = Setter.updatePlayerResult(applicationRoot,
						...
						if (markLevelCompleteTest != null) {
							...
							if (classProgress.indexOf(otherClassName) > 0) {
								...
								String userRowStart = new String(userName
								...
								String firstCharacterOfLength = classProgress.substring(startOfProgressWidth,
										startOfProgressWidth + 1);
								int lengthOfProgress = Integer.parseInt(firstCharacterOfLength); // We dont care what
																									// number it is. As
																									// Long as it's not
																									// 0
						...
						if (markLevelCompleteTest != null) {
							...
							if (jsonProgressString.indexOf(otherClassName) > 0) {
								...
								for (int i = 0; i < jsonProgress.size(); i++) {
									...
									} else if (userProgress.get("userName").toString()
											.compareTo(anotherUserName) == 0) {
										int progressBar = Integer.parseInt(userProgress.get("progressBar").toString());
										if (progressBar != 0) {
				...
				if (Setter.openAllModules(applicationRoot, false) && Setter.openAllModules(applicationRoot, true)) {
					...
					if (!Getter.getModuleAddress(applicationRoot, csrfChallengeTwo, userId).isEmpty()) {
						if (!Getter.isCsrfLevelComplete(applicationRoot, csrfChallengeTwo, userId)) {
							...
							fail("CSRF 2 marked completed without increment"); // CSRF 2 Challenge should have a counter
