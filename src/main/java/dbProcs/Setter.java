...
public class Setter {
		...
		try {
			...
			if (unsafe) {
				PreparedStatement callstmt = conn
						.prepareStatement("UPDATE modules SET moduleStatus = 'open' WHERE isUnsafe = 1");
				callstmt.execute();
				log.debug("All unsafe modules set to open");
			} else {
				PreparedStatement callstmt = conn
						.prepareStatement("UPDATE modules SET moduleStatus = 'open' WHERE isUnsafe = 0");
				callstmt.execute();
	 ...
	 *                        attack under
	 * @param moduleId        The module identifier of which to store the message
	 *                        under
	 ...
	 * @param extra           The additional comments submitted in feedback by the
	 *                        user, or if CSRF, the attack string they used
	 * @param before          The knowledge the user felt before they completed the
	 *                        level
	 * @param after           The knowledge the user felt after they completed the
	 *                        level
	 ...
	 * @param tempPass        Whether or not to set the user with a temporary pass
	 *                        flag
