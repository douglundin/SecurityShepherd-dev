...
public class Validate {
	...
	public static boolean isValidUser(String userName, String passWord, String userAddress) {
		boolean result = false;
		result = userName.length() > 2 && passWord.length() >= 8 && userName.length() <= 32 && passWord.length() <= 512
				...
				if (ses.getAttribute("userRole") != null && ses.getAttribute("userName") != null) {
					try {
						...
						if (!result) {
							...
							if (validCsrfTokens)
								log.fatal("User account " + userName
					...
					try {
						...
						if (!result)
							...
							if (UserKicker.shouldKickUser(userName)) {
								log.debug(userName
										+ " has been Suspended. Invalidating Session and Reporting Invalid Session");
