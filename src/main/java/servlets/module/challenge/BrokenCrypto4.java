		...
		if (Validate.validateSession(ses)) {
			...
			try {
				...
				PreparedStatement prepstmt = conn
						.prepareStatement("SELECT itemId, perCentOff FROM coupons WHERE couponCode = ?");
				...
				// Output Order
				htmlOutput = "<h3>" + bundle.getString("insecureCryptoStorage.4.orderComplete") + "</h3>" + "<p>"
						+ bundle.getString("insecureCryptoStorage.4.orderShipped") + "<br/></p>" + "<p>"
						+ bundle.getString("insecureCryptoStorage.4.totalCost") + " <a><strong>$" + finalCost
						+ "</strong></a></p>";
				if (orangeAmount > 0 && orangeCost == 0) {
					htmlOutput += "<p>" + bundle.getString("insecureCryptoStorage.4.freeOranges") + " - "
