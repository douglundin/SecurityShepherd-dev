...
/**
 * Control class that returns a feedback form for users if they submit the
 * correct solution <br/>
 * <br/>
...
public class SolutionSubmit extends HttpServlet {
			...
			if (Validate.validateTokens(tokenCookie, tokenParmeter)) {
				...
				if (notNull && storedResult != null && moduleOpen) {
					...
					if (Getter.getModuleKeyType(ApplicationRoot, moduleId))
						...
						storedResult = Hash.generateUserSolutionKeyOnly(
					...
					if (validKey) {
						...
						if (result != null) {
							...
							if (FeedbackStatus.isEnabled()) {
								...
								out.write("<h2 class=\"title\">Solution Submission Success</h2><br>"
										+ "<p> You are one step away from completing <a>" + Encode.forHtml(result)
										+ "</a>! To complete the level please submit your feedback!" + "</p><br/>"
							...
							} else // Feedback is disabled
							{
								...
								result = Setter.updatePlayerResult(ApplicationRoot, moduleId, userId,
										"Feedback is Disabled", 1, 1, 1);
								if (result != null) {
									ResourceBundle bundle = ResourceBundle.getBundle("i18n.moduleGenerics.moduleNames",
									...
									htmlOutput = new String("<h2 class=\"title\">Solution Submission Success</h2><br>"
											+ "<p>" + compltedModuleLocalName + " completed! Congratulations.");
									htmlOutput += "</p>";
									// Refresh Side Menu
									htmlOutput += FeedbackSubmit
									...
									out.print("<h2 class=\"title\">Solution Submission Failure</h2><br>"
											+ "<p><font color=\"red\">" + "Sorry but an error Occurred!"
											+ "</font></p>");
								}
							}
						} else {
							log.error("User has completed this module before. Returning Error");
							out.write("<h2 class=\"title\">Haven't You Done This Already?</h2><br>" + "<p>"
						...
						out.print("<h2 class=\"title\">Solution Submission Failure</h2><br>" + "<p><font color=\"red\">"
					...
					out.print("<h2 class=\"title\">Solution Submission Failure</h2><br>" + "<p><font color=\"red\">"
							+ Encode.forHtml(errorMessage) + "</font><p>");
				}

			} else {
				log.debug("CSRF Tokens did not match");
				out.print("<h2 class=\"title\">Solution Submission Failure</h2><br>" + "<p>"
						+ "<font color=\"red\">An error Occurred! Please try again.</font>" + "<p>");
			}
		} else {
			out.print("<h2 class=\"title\">Solution Submission Failure</h2><br>" + "<p>"
					+ "<font color=\"red\">An error Occurred! Please Log in!</font>" + "<p>");
		}
		log.debug("&&& END SolutionSubmit &&&");
	}

	/**
	 * The method responsible for generating the feedback form based on the valid
	...
	private static String generateFeedbackForm(String moduleId, String csrfToken, String theKey) {
		return feedbackForm + Encode.forHtml(moduleId) + feedbackForm21 + Encode.forHtml(theKey) + feedbackForm22
				+ Encode.forHtml(csrfToken) + feedbackForm3;
	}

	// To be written to the user
	private static String feedbackForm = "" + "<div id=\"badData\"></div>\n" + "<div id=\"formDiv\">\n"
